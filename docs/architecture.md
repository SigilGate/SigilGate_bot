...
| Каскад при блокировке пользователя | высокий | user inactive → `devices/deactivate.sh` для каждого устройства |
| Смена имени устройства | средний | Карточка устройства: кнопка «Переименовать» → `devices/update.sh --device` |
| Активация / деактивация устройства пользователем | средний | Карточка устройства: `devices/deactivate.sh`, `entry/add-client.sh` + выбор ноды из `nodes/list-entry.sh --user` |
| `/pending` — список ожидающих одобрения | средний | `users/list.sh --status inactive` + фильтр `core_nodes=[]` |
| Действия в карточке пользователя (admin) | средний | Сменить статус → `users/update.sh`, назначить Core-ноду → `users/update.sh --add-core-node` |
